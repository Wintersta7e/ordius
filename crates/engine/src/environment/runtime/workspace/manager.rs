...
impl WorkspaceManager {
    ...
    async fn reconcile_in_persistent(
        ...
        let (host_at_in, last_remote) =
            persistent::sync_remote_additive(factory, root, host_ws).await?;
    ...
    async fn acquire_persistent_lock(
        ...
        match t.try_acquire_lock_dir(&lock_rel).await? {
            LockOutcome::Acquired => {
                ...
                let owner = persistent::LockOwner {
            ...
            LockOutcome::Contended => {
                ...
                let owner = t
                    ...
                    .await
                    .ok()
                    .and_then(|bytes| serde_json::from_slice::<persistent::LockOwner>(&bytes).ok());
...
async fn reconcile_write_back(
    ...
    match policy {
        ...
        WriteBackPolicy::Force { ignore } => {
            ...
            let advanced_host_at_in = persistent::advance_host_at_in_force(
                &baselines.host_at_in,
                &baselines.remote,
                &new_remote,
            );
...
#[cfg(test)]
mod tests {
    use super::super::persistent::{LockOwner, sync_remote_additive};
