...
pub fn translate_path(distro: &str, host_path: &Path) -> Result<EnvPath, DispatchError> {
    let s = host_path
        .to_str()
        .ok_or_else(|| DispatchError::Translation("host path not valid UTF-8".into()))?;
    ...
    if let Some((other_distro, rest)) = strip_wsl_prefix(s) {
        if other_distro != distro {
            return Err(DispatchError::Translation(format!(
    ...
    Err(DispatchError::Translation(format!(
        "no inline rule maps host path `{s}` into WSL distro `{distro}`; \
...
pub async fn translate_path_via_wslpath(
    ...
    let out = cmd
        .output()
        .await
        .map_err(|e| DispatchError::Translation(format!("wslpath spawn failed: {e}")))?;
    if !out.status.success() {
        return Err(DispatchError::Translation(format!(
            "wslpath -u exited with {:?}",
            out.status.code()
        )));
    }
    let text = String::from_utf8_lossy(&out.stdout).trim().to_string();
    if text.is_empty() {
        return Err(DispatchError::Translation(
            "wslpath -u produced empty output".into(),
        ));
    }
    Ok(EnvPath::new(text))
}
...
mod tests {
    ...
    fn wsl_prefix_with_wrong_distro_errors() {
        ...
        assert!(matches!(err, DispatchError::Translation(_)));
    }
