//! Process-wide `reqwest::Client` shared by every built-in that talks HTTP.
//!
//! One client = one TLS session cache + DNS resolver + connection pool,
//! so an `http` node and an `llm` node hitting the same host reuse the
//! same underlying TCP connections.
//!
